...
Forwarder::Forwarder(FaceTable& faceTable)
  ...
  , m_tfib(getScheduler())
  , m_floodRateWindowStart(time::steady_clock::now())
{
                              ...
                              const shared_ptr<pit::Entry>& pitEntry)
{
  ...
  // OptoFlood: First check TFIB for temporary paths
  ...
  // OptoFlood: Check if we should trigger flooding
...
Forwarder::onIncomingData(const Data& data, const FaceEndpoint& ingress)
{
  ...
  // OptoFlood: Check if this is a mobility-related Data packet
  if (ndn::optoflood::hasMobilityFlag(data.getMetaInfo())) {
    handleOptoFloodData(data, ingress);
  }
