...
NFD_PUBLIC_WITH_TESTS_ELSE_PRIVATE: // pipelines
  ...
  */
  ...
  // OptoFlood methods
  /** \brief Handle OptoFlood Data packet
   */
  void
  handleOptoFloodData(const Data& data, const FaceEndpoint& ingress);

  /** \brief Check if Interest should trigger flooding
   */
  bool
  shouldFloodInterest(const Interest& interest) const;

  /** \brief Handle Interest flooding for OptoFlood
   */
  void
  handleInterestFlooding(const Interest& interest, const FaceEndpoint& ingress,
                        const shared_ptr<pit::Entry>& pitEntry);

  /** \brief Check if face should be used for guided flooding
   */
  bool
  shouldUseGuidedFlooding(const Face& face, const std::vector<uint8_t>& traceHint) const;
