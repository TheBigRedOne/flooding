...
if [ -n "${NLSR_TUNING_PROFILE:-}" ]; then
  ...
  REMOTE_MAKE_CMD="NLSR_HELLO_INTERVAL=$(shell_quote "$NLSR_HELLO_INTERVAL") \
NLSR_ADJ_LSA_BUILD_INTERVAL=$(shell_quote "$NLSR_ADJ_LSA_BUILD_INTERVAL") \
NLSR_ROUTING_CALC_INTERVAL=$(shell_quote "$NLSR_ROUTING_CALC_INTERVAL") \
NLSR_TUNING_PROFILE=$(shell_quote "$NLSR_TUNING_PROFILE") \
make all"
else
