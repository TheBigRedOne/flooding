...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Run NDN mobility experiment')
    parser.add_argument('--mode', choices=['baseline', 'solution'], 
                        default=os.environ.get('APP_MODE', 'baseline'),
                        help='Run in baseline or solution mode')
    ...
    # Add delay between NFD and NLSR startup
    sleep(3)
    ...
    sleep(35)  # Wait for NLSR to stabilize
    ...
    producer.cmd(f"tcpdump -i producer-eth0 -w {base_dir}/producer_capture.pcap &")

    # Sleep before starting apps to ensure NLSR is ready
    sleep(5)

    info(f'Starting applications in {args.mode} mode\n')
    consumer.cmd(f"{base_dir}/consumer &> {base_dir}/consumer.log &")
    producer.cmd(f"{base_dir}/producer --mode {args.mode} &> {base_dir}/producer.log &")
    sleep(120)
