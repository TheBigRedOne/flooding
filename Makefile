...
# Nodes to export logs for (mobility-related nodes in experiment/tool/exp.py).
MOBILITY_LOG_NODES ?= producer acc2 acc3 acc4
LOG_EXPORT_BASELINE_DIR := experiment/baseline/minindn-logs
LOG_EXPORT_SOLUTION_DIR := experiment/solution/minindn-logs
...
$(CSV_BASELINE): $(APP_SRCS) $(BASELINE_SRCS) $(TOOLS_SRCS) .ssh_config_baseline | results/baseline
	...
	mkdir -p $(LOG_EXPORT_BASELINE_DIR)
	$(RSYNC_BASELINE) baseline:$(REMOTE_DIR)/experiment/baseline/results/minindn-logs/ $(LOG_EXPORT_BASELINE_DIR)/
...
$(CSV_SOLUTION): $(APP_SRCS) $(SOLUTION_SRCS) $(TOOLS_SRCS) .ssh_config_solution | results/solution
	...
	mkdir -p $(LOG_EXPORT_SOLUTION_DIR)
	$(RSYNC_SOLUTION) solution:$(REMOTE_DIR)/experiment/solution/results/minindn-logs/ $(LOG_EXPORT_SOLUTION_DIR)/
...
clean: clean-ssh-config
	...
	rm -rf $(LOG_EXPORT_BASELINE_DIR) $(LOG_EXPORT_SOLUTION_DIR)
