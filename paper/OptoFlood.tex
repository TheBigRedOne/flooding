...
These flooded Data packets preferentially follow existing valid FIB next hops; if no valid FIB next hop exists, they fall back to forwarding over directly-adjacent faces (i.e., data-plane neighbours) under strict scope controls. In our implementation the Data hop count is carried in the NDNLPv2 header as an experimental \emph{OptoHopLimit} field (value~96), which is outside the signed portion of the Data and is decremented hop-by-hop. We conservatively bound propagation with a maximum of three hops, restricting flooding within typical local access and aggregation networks. The goal is to reach nodes maintaining valid Pending Interest Table (PIT) entries and promptly rescue pending Interests.

\subsection{Interest Flooding Triggered by Forwarder FIB Invalidation}
\label{sec:solution:interest-flooding}

Forwarders conditionally perform Interest flooding when an Interest arrives without available next hops (FIB/TFIB miss), or when an explicit flood request is present. In our prototype, the consumer includes an explicit request in the Interest \emph{ApplicationParameters} after a small number of consecutive failures; the forwarder acts as the enforcement point and sets the native \emph{HopLimit} before executing flooding, and only under FIB/TFIB miss. Optionally, a fast, rate-limited probe on the first miss can be enabled to further reduce recovery time.
The Interest flooding strategy involves:
\begin{itemize}
\item \textbf{Prioritised Forward}: Forwarders first attempt to forward Interests via existing valid FIB entries towards the last known PoAs, guided by \textit{Trace-Hints} embedded within Interest packets.
...
% rationale for TFIB vs FIB

The TFIB entries maintain temporary forwarding state with a brief TTL (approximately one second), chosen to bridge micro-mobility gaps without accumulating state; entries naturally expire as regular routing reconverges, minimising memory and control overhead.
...
OptoFlood integrates temporary flooding-based paths with global routing convergence via Named-data Link State Routing (NLSR) \cite{}. Whenever a forwarder establishes a TFIB entry due to successful flooding, it triggers NLSR over the local management namespace to publish a short-lived Fast-LSA (about one second). The trigger conveys \textit{Name}, \textit{ExpirationPeriod}, optionally \textit{FaceId}, and encodes \textit{NewFaceSeq} using the cost field. Upon trigger, NLSR installs a FastPrefixLsa and performs short-lived RIB registration via its controller interface, and actively unregisters at expiry. Simple 500~ms per-prefix throttling and Prefix+NewFaceSeq deduplication prevent churn. TFIB-based triggering ensures that only verified, data-plane-confirmed paths seed convergence, while regular periodic LSAs later supersede these temporary hints.
...
First, NDN Data packets contain digital signatures that authenticate static packet fields such as names and content. The hop-by-hop \textit{OptoHopLimit} used for Data flooding is carried in the NDNLPv2 header and is not covered by the signature, so decrementing it at each hop does not affect signature verification. Similarly, dynamic indicators such as \textit{MobilityFlag} are used for forwarding decisions and are not intended to be protected end-to-end.
