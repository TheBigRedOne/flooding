...
\begin{figure}
    \centering
    \includegraphics[width=\columnwidth]{baseline_nlsr_disruption_comparison.pdf}
    \caption{Service disruption under baseline NDN forwarding with different NLSR parameter settings.}
    \label{fig:baseline_nlsr_disruption_comparison}
\end{figure}

\begin{figure}
    \centering
    \includegraphics[width=\columnwidth]{baseline_nlsr_network_cost_comparison.pdf}
    \caption{Forwarding and NLSR control cost under the same baseline parameter settings.}
...
\csp{In the following paragraph, ``marks the Data packets associated with the mobility event'' is imprecise. Should it be something like ``identifies Data packets that will be sent after the change in attachment point but in response to an Interest received before the change''? Also need to cross-reference the earlier section with details of the mechanism.}
Upon detecting changes in its network attachment point, the producer marks the Data packets associated with the mobility event to enable OptoFlood to control the flooding and acceleration of convergence. In our deployment, immutable identifiers used for flooding (e.g., Flood-ID and a sequence number for forwarding hints) are carried in signed Data MetaInfo extension TLVs, while hop-by-hop controls (e.g., mobility indication and scoped hop limits) are carried in the LP header and may be updated by intermediate forwarders.
...
We additionally retain packet traces on all relay and endpoint nodes for analysis.
\csp{What is a relay node? This term has not been defined.}
...
\begin{itemize}
    ...
    \item \textbf{Service disruption time:} for each hand-off, the time between the last Data packet observed before the move and the first Data packet observed after service resumes. \csp{Observed where?}
    \item \textbf{Unmet-interest ratio:} an Interest is counted as unmet if the first request for a content name is not satisfied by Data within the application deadline; we evaluate this in the 10s interval immediately after each hand-off and compare it with the steady-state remainder of the run. \csp{A ratio compares two quantities: what are the unmet Interests compared with?}
    \item \textbf{Forwarding overhead:} application relay load across the relay nodes, normalised by delivered Data bytes, together with a decomposition into explicit flooding traffic and NLSR control traffic. \csp{What is a relay node? This term has not been defined.}
\end{itemize}
...
Under baseline forwarding, each hand-off is followed by a sharp drop in delivered throughput. The consumer continues to issue Interests, but receives little or no Data during the outage interval. Recovery appears only after routing convergence catches up. Figures~\ref{fig:baseline_throughput} and~\ref{fig:baseline_disruption} show disruption on the order of tens of seconds.

\begin{figure}
    \centering
    \includegraphics[width=\columnwidth]{baseline_throughput.pdf}
    \caption{Throughput continuity under baseline NDN forwarding with the default NLSR parameter setting.}
...
\begin{figure}
    ...
    \caption{Per-handoff service disruption under baseline NDN forwarding with the default NLSR parameter setting.}
    \label{fig:baseline_disruption}
\end{figure}

With OptoFlood, the stream remains continuous except for a narrow disturbance around each hand-off. Figure~\ref{fig:solution_disruption} shows recovery on the order of tens of milliseconds. The throughput trace in Figure~\ref{fig:solution_throughput} is correspondingly stable across the mobility events.

\begin{figure}
    \centering
    \includegraphics[width=\columnwidth]{solution_disruption.pdf}
    \caption{Per-handoff service disruption with OptoFlood. \csp{This plot is light on information. Is it possible to plot service disruption time against time taken for the handover to complete, or variation with topology changes, or something?}}
    \label{fig:solution_disruption}
\end{figure}

\begin{figure}
    \centering
    \includegraphics[width=\columnwidth]{solution_throughput.pdf}
    \caption{Throughput continuity with OptoFlood.}
    \label{fig:solution_throughput}
\end{figure}


\subsubsection{Delivery Reliability}

The unmet-interest results follow the same trend. In the baseline case, the immediate post-handoff window contains a high fraction of unsatisfied requests. The consumer continues to issue Interests, but stale forwarding state prevents many of them from being satisfied before their deadline. Figure~\ref{fig:baseline_loss} captures this behaviour directly.

Under OptoFlood, the unmet-interest ratio in the hand-off window falls close to the steady-state level, as shown in Figure~\ref{fig:solution_loss}. This indicates that temporary flooded paths and short-lived forwarding hints are sufficient to sustain delivery while routing converges. The steady-state ratio remains low as well. Normal forwarding therefore remains stable after a valid path has been re-established.
\csp{Again, these plots would be more convincing if they showed a trend as handover time changed, or as the topology changed, or similar.}

\begin{figure}
    \centering
    \includegraphics[width=\columnwidth]{baseline_loss.pdf}
    \caption{Unmet-interest ratio under baseline forwarding.}
    \label{fig:baseline_loss}
\end{figure}
\begin{figure}
    \centering
    \includegraphics[width=\columnwidth]{solution_loss.pdf}
    \caption{Unmet-interest ratio with OptoFlood.}
    \label{fig:solution_loss}
\end{figure}

\subsubsection{Forwarding Overhead and Control Activity}

The overhead figures report network activity of relay load. Figures~\ref{fig:baseline_overhead_timeseries} and~\ref{fig:solution_overhead_timeseries} show the non-flood and flooded relay traffic for Interests and Data, and also show the NLSR control messages. Figures~\ref{fig:baseline_overhead_summary} and~\ref{fig:solution_overhead_summary} summarise the same quantities over the hand-off windows and the full run. The summary panels also report the forwarding-cost ratio and flood share for each window.
...
The baseline overhead figures show a different pattern. In the immediate post-handoff windows, the consumer may receive no Data. The forwarding-cost ratio is therefore undefined in those windows, as shown in Figure~\ref{fig:baseline_overhead_summary}. This reflects the severity of the disruption. Figure~\ref{fig:baseline_overhead_timeseries} still shows routing control activity, but it tends to appear later, after the disruption has already persisted for much longer.

Taken together, the overhead results are consistent with the design target. OptoFlood introduces additional traffic during recovery. That traffic is temporally localised and remains bounded. The figures therefore indicate that the gain in continuity is achieved through short-lived overhead around mobility events, rather than through a persistent increase in relay load.

\begin{figure}
    \centering
    \subfloat[Time-series decomposition by packet type.\label{fig:baseline_overhead_timeseries}]{
        \includegraphics[width=\columnwidth]{baseline_overhead_timeseries.pdf}
    }

    \subfloat[Window summaries by packet type.\label{fig:baseline_overhead_summary}]{
        \includegraphics[width=\columnwidth]{baseline_overhead_summary.pdf}
    }
...
\begin{figure}
    ...
    \subfloat[Time-series decomposition by packet type.\label{fig:solution_overhead_timeseries}]{
        \includegraphics[width=\columnwidth]{solution_overhead_timeseries.pdf}
    }

    \subfloat[Window summaries by packet type.\label{fig:solution_overhead_summary}]{
        \includegraphics[width=\columnwidth]{solution_overhead_summary.pdf}
    }
    \caption{Forwarding and control load with OptoFlood.}
    \label{fig:solution_overhead}
\end{figure}

Overall, the evaluation shows that OptoFlood shortens producer hand-off disruption from long outages to brief transients. It keeps unmet requests close to the steady-state regime during mobility. The additional forwarding and control traffic remains bounded.
