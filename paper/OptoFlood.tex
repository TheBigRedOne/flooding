...
In baseline forwarding, each hand-off is followed by a sharp drop in delivered throughput. The consumer continues to issue Interests, but receives little or no Data during the outage interval. Recovery appears only after routing convergence catches up. With OptoFlood, the stream remains continuous except for a narrow disturbance around each hand-off. Figure~\ref{fig:service_continuity_comparison} compares the throughput traces and the disruption times per-handoff.

\begin{figure*}[t]
    \centering
    \includegraphics[width=0.92\textwidth]{service_continuity_comparison.pdf}
    \caption{Service continuity under baseline forwarding and OptoFlood%
	\ryo{Don't try to do sub figures at the time of generating the plot itself}%
	}
    \label{fig:service_continuity_comparison}
\end{figure*}
