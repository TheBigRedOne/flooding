...
\csp{As discussed, I don't think this is the right place for these results.
Maybe move these baseline results to \ref{sec:evaluation}, which would then
have subsections for Methodology, Baseline Results, OptoFlood Results?}

\csp{I'm not sure I follow Figure \ref{fig:baseline_throughput}, for three
reasons: 1) It looks like the handover takes 10 seconds to complete? Why?;
2) Why does the throughput reduce but not drop to zero after the handover?;
and 3) Why does the throughput take 30 seconds to recover? It looks like
something is misconfigured with the NLSR routing if it takes this long to
react to the change. Can parameters be tuned?}

\begin{figure}
    \centering
    \includegraphics[width=\columnwidth]{baseline_throughput.pdf}
    \caption{Baseline Throughput Disruption \csp{Text in the figure is too
small to read} %
      \ryo{throughput needs to split between interest and data loads}%
    }
...
\begin{figure}
    ...
    \caption{Baseline Disruption Timeline \csp{Text in the figure is too
small to read}}
