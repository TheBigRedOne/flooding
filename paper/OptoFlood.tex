...
We consider producer micro-mobility within a single access domain, where an ongoing real-time stream remains active while the producer changes its attachment point. This setting stresses the interval between the physical move and routing convergence, which is precisely where NDN producer mobility causes service interruption.
...
The experiments are run in Mini-NDN 0.7.0 with identical topology, applications, and hand-off schedule in two configurations: baseline NDN forwarding and OptoFlood-enabled forwarding \cite{lantz:2010:mininet}. In each run, all nodes start NFD and NLSR and are given time to converge before the applications are launched. The producer then performs two scheduled hand-offs, separated by long enough intervals to let the transient behaviour around each event be observed clearly.

We record packet traces (\texttt{ndndump}) in three places: the consumer, the access switch adjacent to the old PoA, and the access switch adjacent to the new PoA for each hand-off. In the implementation used to regenerate the figures in this repository, we additionally retain packet traces on all relay and endpoint nodes and merge them into a unified packet-level CSV for the overhead analysis, so that hop-by-hop forwarding cost can be reconstructed consistently from fresh runs.
...
\begin{itemize}
    ...
    \item \textbf{Service disruption time:} for each hand-off, the time between the last Data packet observed before the move and the first Data packet observed after service resumes.
    \item \textbf{Unmet-interest ratio:} an Interest is counted as unmet if the first request for a content name is not satisfied by Data within the application deadline; we evaluate this in the 10~s interval immediately after each hand-off and compare it with the steady-state remainder of the run.
...
Because the figures are regenerated from fresh experiment runs, small run-to-run variation is expected. We therefore emphasise stable trends and relative magnitudes rather than over-interpreting exact point values.
