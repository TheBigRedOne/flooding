...
The unmet-interest results follow the same trend. In the baseline case, the immediate post-handoff window contains a high fraction of unsatisfied requests. The consumer continues to issue Interests, but stale forwarding state prevents many of them from being satisfied before their deadline. Under OptoFlood, the unmet-interest ratio in the hand-off window falls close to the steady-state level. Figure~\ref{fig:unmet_interest_comparison} compares both regimes directly. This indicates that temporary flooded paths and short-lived forwarding hints are sufficient to sustain delivery while routing converges. The steady-state ratio remains low as well, indicating that normal forwarding remains stable after a valid path has been re-established.
