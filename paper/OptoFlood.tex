...
\begin{enumerate}
\item \emph{Data packet flooding}, for Data packets packets marked as being sent after the producer moves, in response to pending Interest packets received before the move.
This flooding is explicitly hop-limited and rate controlled, and is intended to quickly ``push'' Data across nearby nodes so that Interests that would otherwise stall can be satisfied without waiting for global routing convergence.
