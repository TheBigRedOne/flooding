...
NDN shifts from a host-centric model to a data-centric model, where content is identified by names rather than addresses \cite{trossen:2012:icn-internet, xylomenos:2014:icn-survey}. In NDN, communication relies on two core packet types: Interest Packets, sent by consumers to request data by specifying the content name, and Data Packets, returned by producers containing the requested content and related metadata. Both types of packets follow a concise Type-Length-Value (TLV) encoding, allowing efficient processing and extendability \cite{jacobson:2009:named-content, zhang:2014:ndn}.
...
NLSR has a number of tuneable parameters that can reduce convergence times.  We explore the effects of changing these parameters in the following, measuring baseline forwarding under three NLSR configurations that progressively reduce the hello interval, adjacency-LSA build interval, and routing calculation interval. Figure~\ref{fig:baseline_nlsr_disruption_comparison} shows that the lowest-interval configuration tested (36/6/9~s) reduces the observed interruption relative to the default configuration in some hand-offs, but still leaves service disruption on the order of tens of seconds. Figure~\ref{fig:baseline_nlsr_network_cost_comparison} shows the corresponding forwarding and control-plane cost. Across the tested configurations, the full-run forwarding-cost ratio remains close to 5.1--5.2, while NLSR control traffic increases as intervals are shortened.
...
\begin{figure}
    \centering
    \includegraphics[width=\columnwidth]{results/baseline/disruption_comparison.pdf}
    \caption{Service disruption under baseline NDN forwarding}
    \label{fig:baseline_nlsr_disruption_comparison}
\end{figure}

\begin{figure}
    \centering
    \includegraphics[width=\columnwidth]{results/baseline/network_cost_comparison.pdf}
    \caption{Forwarding and NLSR control cost}
    \label{fig:baseline_nlsr_network_cost_comparison}
\end{figure}

These issues highlight the limitations of NLSR in handling frequent producer mobility. Addressing these issues effectively requires specialised mechanisms that can rapidly establish temporary communication paths without waiting for global routing convergence.
