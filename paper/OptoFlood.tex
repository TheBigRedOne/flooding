...
The results consistently show the same overall pattern across reruns: baseline NDN experiences long hand-off outages while OptoFlood converts those outages into short transients, and it does so without causing an uncontrolled increase in network load.

\subsubsection{Service Continuity}

Under baseline forwarding, each hand-off produces a pronounced collapse in delivered throughput and a long interval during which the consumer continues to send Interests but receives little or no Data. Recovery is only observed after ordinary routing convergence catches up, which leads to disruption on the order of tens of seconds, as illustrated by the baseline throughput and disruption plots in Figures~\ref{fig:baseline_throughput} and~\ref{fig:baseline_disruption}.

With OptoFlood, the behaviour is qualitatively different. The stream remains continuous apart from a very narrow disturbance around each hand-off, and the disruption plot in Figure~\ref{fig:solution_disruption} shows recovery on the order of tens of milliseconds rather than tens of seconds. In other words, the producer move is no longer visible as a prolonged service outage but as a brief perturbation. The aggregate delivered throughput over the full run also increases, because the long blind period that dominates the baseline run is largely removed.
...
The unmet-interest results show the same transition from failure-dominated recovery to near-continuous delivery. In the baseline case, the immediate post-handoff window is dominated by unsatisfied requests: the consumer continues to issue Interests, but stale forwarding state prevents them from being satisfied before their deadline. This is exactly the failure mode expected from pure routing-plane recovery.

Under OptoFlood, the hand-off window ratio drops to near the steady-state regime, showing that the temporary flooded paths and short-lived forwarding hints are effective in rescuing pending requests while routing converges. Just as importantly, the steady-state unmet-interest ratio remains low and similar to the baseline case, indicating that the mechanism does not destabilise normal forwarding once a valid path has been restored.
...
The overhead figures must be read as a decomposition of network activity, not simply as a count of flooded packets. The main time series shows the total application relay load, the explicit flood component, and the NLSR control component, all observed across the relay nodes.

Several features are noteworthy. First, the full-run forwarding cost under OptoFlood remains in the same range as the baseline. This means the gain in service continuity is not bought by an uncontrolled explosion of hop-by-hop forwarding load. Second, the explicit flood component is concentrated around the hand-offs, where path repair is actually needed, and remains a small fraction of the total application forwarding load over the whole run. Third, the control-plane activity appears earlier and more strongly under OptoFlood, which is expected because Fast-LSA and related NLSR activity are triggered promptly once useful temporary paths are discovered.

The baseline overhead plot requires a slightly different interpretation. In the immediate post-handoff windows, the consumer may receive no Data at all, so a normalised forwarding-cost ratio for those windows is undefined. This is not a plotting artefact; it is itself evidence of a severe service outage. The baseline still shows routing control activity, but it tends to appear later, after service has already been disrupted for much longer. By contrast, OptoFlood keeps delivery active during the same windows, so the forwarding-cost ratio remains meaningful even while recovery is in progress.

Taken together, the overhead results support the intended design point: OptoFlood adds extra traffic where recovery is needed, but that traffic is temporally localised, operationally bounded, and comparable in overall scale to the forwarding cost already incurred by the baseline during prolonged disruption and retransmission.

\begin{figure}
    \centering
    \includegraphics[width=\columnwidth]{baseline_overhead.pdf}
    \caption{Forwarding and control load under baseline forwarding.%
    \ryo{1. These needs to be a sub figure, also have consistent y-axis limits with upper plot in Figure \ref{fig:solution_overhead}.}%
     \ryo{2. Throughput needs to be split between interest and data.}%
     }
...
Overall, the evaluation shows that OptoFlood converts producer hand-offs from long user-visible outages into brief transients, keeps unmet requests near the steady-state regime during mobility, and does so with additional forwarding and control traffic that remains bounded rather than escalating uncontrollably.
